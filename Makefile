...
# QEMU 模拟器配置 (优先使用 QEMU 5.1，否则自动检测)
QEMU     ?= $(shell \
    if [ -x /usr/local/qemu-5.1/bin/qemu-system-riscv64 ]; then echo /usr/local/qemu-5.1/bin/qemu-system-riscv64; \
    else which qemu-system-riscv64 2>/dev/null || echo qemu-system-riscv64; fi)
...
# 运行目标 (每次运行前重建磁盘镜像以确保干净状态)
.PHONY: run
run: build
	rm -f $(DISKIMG)
	$(TARGET)/mkfs/mkfs $(DISKIMG) $(USER_TEST_ELF)
	$(QEMU) $(QEMUOPTS)
